...
.defines( function(){
	...
	EntityPlayer = ig.Entity.extend({
		...
		size: {x:20,y:40},		/* size is 20 by 40*/
		offset:{x:10,y:20},		/* collision box is a little bit bigger */
		...
		update: function(){
			this.parent();	//use the defaults
	         var anyInput = ig.input.state('up') || ig.input.pressed('tbUp') ||ig.input.state('down') || ig.input.pressed('tbDown') ||
                        ig.input.state('left') || ig.input.pressed('tbLeft') || ig.input.state('right')||ig.input.pressed('tbRight');
			//player movement
            ...
            else if(ig.input.pressed("attack2")){
                //console.log("attack2 fired");
                if (!ig.ua.mobile){
                    ig.game.spawnEntity('EntityProjectile',this.pos.x,this.pos.y,
                                        {player: this, direction:this.lastPressed, 
                                         clickx: ig.input.mouse.x, 
                                         clicky: ig.input.mouse.y});
                }
                else{
                    ig.game.spawnEntity('EntityProjectile',this.pos.x,this.pos.y,
                                        {player: this, direction:this.lastPressed, 
