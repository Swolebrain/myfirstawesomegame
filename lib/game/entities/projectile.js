ig.module('game.entities.projectile'
  ...
  .defines( function(){
	EntityProjectile = ig.Entity.extend({
		...
		size: {x:8,y:4},
		vel:  {x:100,y:0},
        maxVel: {x: 300, y: 300}, //THIS FIXED THE INACCURATE PATHING
        speed: 400,
		bounciness:1,
		lifetime: 0,
		...
		init: function(x, y , settings){
			...
			var player = this.player; //ig.game.getEntitiesByType( EntityPlayer )[0];
			...
			var xpos = this.clickx - ig.system.width/2; //ig.system is a ref to the canvas
            var ypos = -this.clicky + ig.system.height/2;
            var a = Math.atan(ypos/xpos);
            if (xpos < 0 && y > 0) a += Math.PI;
            //if (xpos < 0 && ypos < 0) a = Math.PI*2 - a;
            //console.log("angle: "+a+" x:"+xpos+", y: "+ypos);
            this.vel.x = Math.cos(a)*this.speed;
            this.vel.y = -Math.sin(a)*this.speed;
            this.anims.xaxis.angle = -a;
            ...
            /*if(this.direction =='right'){
                ...
                this.anims.xaxis.angle = Math.PI/2;
			}
